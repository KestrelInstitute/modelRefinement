...
Note: hist only needed in order to state the reqd props to the model checker. 
It would not be part of the model produced by m.r.
*)
...
in ATLAS: UnlockPre == Unlock => (<_>_K Insert)
after prop transforms *)
UnlockPre == [][Unlock => clock <= lastInsertT + K]_vars
(*and requires
    [](Insert => lastInsertT' = now)
  but this isn't stuttering invs so need to write*)
LastInsertTProp == [][Insert => lastInsertT' = clock]_vars
...
\*AnyAction = Unlock \/ Lock \/ Insert \/ Remove
\*foo == [][AnyAction => last(timestamps')=clock]_vars
...
\* Last modified Mon Oct 04 20:22:45 PDT 2021 by snedunu
